...
\id{RF-08} & Diagnóstico nutricional por imagen & 2,25 & \textbf{Simulado} & Análisis \\ \hline
\id{RF-10} & Gestión de variedades y umbrales & 7,00 & Implementado & --- \\ \hline
\id{RF-12} & Generación de alertas tempranas & 4,20 & Implementado & Alertas \\ \hline
\id{RF-13} & Registro del proceso de polinización & 4,20 & Implementado & Labores \\ \hline
\id{RF-14} & Conteo georreferenciado de flores & 3,25 & \textbf{Simulado} & Mapa GPS \\ \hline
\id{RF-18} & Estimación de producción y rendimiento & 3,60 & Implementado & --- \\ \hline
\id{RF-19} & Generación de reportes & 3,20 & Implementado & Reportes \\ \hline
\id{RF-21} & Clasificación de madurez del racimo & 4,25 & \textbf{Simulado} & --- \\ \hline
\id{RF-22} & Alerta preventiva de fruta verde & 4,25 & Implementado & --- \\ \hline
\id{RF-26} & Planificación semanal con presupuesto & 3,60 & Implementado & --- \\ \hline
\id{RF-28} & Registro de avance por unidad de labor & 3,60 & Implementado & Labores \\ \hline
\id{RF-30} & Reporte de incidencia desde campo & 4,80 & Implementado & Análisis \\ \hline
\id{RF-35} & Registro delegado del avance & 7,00 & Implementado & --- \\ \hline
\id{RF-36} & Catálogo de tarifas por labor & --- & Implementado & --- \\ \hline
\id{RF-37} & Cálculo de la remuneración semanal & --- & Implementado & --- \\ \hline
\id{RF-40} & Exportación de datos personales & --- & Implementado & --- \\ \hline
\id{RF-41} & Rectificación con bitácora & --- & Implementado & --- \\ \hline
\id{RF-42} & Supresión/disociación del histórico & --- & Implementado & --- \\ \hline
\caption{Cobertura verificada del MVP V2 sobre los 24 requisitos funcionales \emph{Must}}
\label{tab:mvp}
\end{longtable}

\noindent
\textbf{Cobertura alcanzada.} La verificación de la versión V2 del prototipo contra el catálogo
vigente posterior al Change Control Board identifica los 24 requisitos funcionales \emph{Must}
representados en el prototipo. De ellos, 20 cuentan con flujo funcional y cuatro se mantienen como
interfaz o comportamiento simulado: \id{RF-07}, \id{RF-08}, \id{RF-14} y \id{RF-21}. Estos cuatro
últimos no se contabilizan como funcionales. En consecuencia, la cobertura funcional verificable
es de $20/24 = 83{,}3\%$, superior al umbral mínimo del 80\% establecido para el criterio C3.

\noindent\fbox{\parbox{0.97\textwidth}{\small\textbf{Nota histórica de corrección (v3.1, MVP V1).}
La versión 3.0 declaraba una cobertura de $9/19 = 47{,}4\%$. La revisión v3.1 corrigió ese cálculo
a $8/21 = 38{,}1\%$ para el alcance existente durante la construcción de la V1 y a
$8/24 = 33{,}3\%$ frente al catálogo post-CCB. Esas cifras se conservan únicamente como
trazabilidad histórica. La cobertura vigente del prototipo V2 es la verificada en la tabla anterior:
$20/24 = 83{,}3\%$, sin contabilizar como funcionales los cuatro requisitos simulados.}}

\noindent\fbox{\parbox{0.97\textwidth}{%
\small\textbf{Declaración de cumplimiento del mínimo.} La §3.6 de la guía exige una cobertura de
al menos el 60\% de los requisitos \emph{Debe tener}. La versión V2 alcanza una cobertura funcional
verificable de $20/24 = 83{,}3\%$, sin contabilizar como implementadas las funcionalidades
simuladas. Por tanto, supera tanto el mínimo de la §3.6 como el umbral del 80\% utilizado para el
criterio C3. La limitación histórica asociada se conserva como \id{L-10}, marcada como resuelta.}}

\subsubsection{Funcionalidades simuladas}

Cuatro requisitos presentan interfaz o comportamiento demostrativo sin procesamiento real, y así se
declaran tanto en este documento como en el archivo \id{README.md} del repositorio del prototipo:

\begin{itemize}[nosep]
  \item \textbf{Detección de plagas por imagen (\id{RF-07}).} La interfaz permite recorrer el flujo
        de análisis, pero no ejecuta inferencia real sobre la imagen.
  \item \textbf{Diagnóstico nutricional por imagen (\id{RF-08}).} El flujo visual está representado,
        pero no existe un modelo entrenado del dominio que produzca un diagnóstico real.
  \item \textbf{Conteo georreferenciado (\id{RF-14}).} El mapa emplea información demostrativa y
        no consulta necesariamente el receptor GPS real del dispositivo.
  \item \textbf{Clasificación de madurez del racimo (\id{RF-21}).} El flujo de clasificación se
        presenta de forma demostrativa y no ejecuta inferencia visual real sobre el racimo.
\end{itemize}
...
\textbf{Especificación.} Se pasó de 20 a 42 requisitos funcionales, de 9 a 19 requisitos no
...
\textbf{Trazabilidad.} La matriz enlaza nueve niveles en 55 filas con trece columnas. Se corrigió la
...
\id{L-06} & Las sesiones de validación \emph{walkthrough} realizadas son inferiores a las tres exigidas: se realizó 0 de 3. & Se ejecutarán antes de la Entrega 4. A esta limitación se suma, en sentido contrario, la inspección formal tipo Fagan ejecutada en la PE4 sobre las §1 a §7 (63 páginas efectivas), cuyo registro de defectos constituye una validación documentada adicional. \\ \hline
\id{L-07} & \emph{Resuelta en v3.0; se conserva la fila por trazabilidad histórica y no se contabiliza como limitación vigente.} La obtención de la hoja de liquidación semanal (\id{EV-13}) incorporó un segundo tipo documental, distinto del plan de labores. & Se dispone de dos tipos documentales de la organización. La obtención de un tercer tipo (hoja de conteo del polinizador o boleta de pesaje) queda como trabajo pendiente. \\ \hline
\id{L-10} & \emph{Resuelta en MVP V2; se conserva la fila por trazabilidad histórica y no se contabiliza como limitación vigente.} La V1 alcanzaba una cobertura funcional de 38{,}1\% sobre 21 requisitos \emph{Must}. & La V2 alcanza $20/24 = 83{,}3\%$ de cobertura funcional sobre el catálogo vigente. \id{RF-07}, \id{RF-08}, \id{RF-14} y \id{RF-21} permanecen simulados y no se contabilizan como funcionales. \\ \hline
\id{L-12} & No se ha declarado el marco de siembra de la plantación, dato necesario para determinar si la precisión GPS de \id{RNF-05} ($\leq 10$ m) permite discriminar palmas contiguas en \id{RF-14}. & \id{RF-14} se acota en v3.1 a la agregación por línea de siembra. El dato se solicitará a la organización antes de la Entrega 4. \\ \hline
...
\noindent\textbf{Estado del depósito FAIR.} La estadística descriptiva agregada de la encuesta fue publicada en Zenodo el 1 de septiembre de 2026, con DOI de versión \id{10.5281/zenodo.22236500} y DOI conceptual \id{10.5281/zenodo.22236499}, bajo licencia CC BY 4.0 \cite{wilkinson2016}.
